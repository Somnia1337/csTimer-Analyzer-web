...
- A new analysis option `dnfasok` to treat DNF records as OK (only for mean & average) (`#7`).
...
- Failed to analyze data files larger than 10MB (now the limit is 256MB).
...
- Grouping interval `0` is now treated as "auto", which means the analyzer will decide a proper interval based on session data.
  - It tries to make number of groups closest to a preset value.

### Fixed

- Option `group(stats, 0)` triggers a panic.
...
- A new analysis option `recent` to stat (only `summary`) recent solves (`#1` `#2`), it takes the form of:
